...
def parse_arguments(mode_name: str) -> dict | None:
    ...
    if args.print_config:
        ...
        Logger.INFO(f"Data Rate: {"250KBPS" if args.data_rate == RF24_DATA_RATE.RATE_250KBPS else "1MBPS" if args.data_rate == RF24_DATA_RATE.RATE_1MBPS else "2MBPS"}")
        Logger.INFO(f"PA Level: {"MIN" if args.pa_level == RF24_PA.MIN else "LOW" if args.pa_level == RF24_PA.LOW else "HIGH" if args.pa_level == RF24_PA.HIGH else "MAX"}")
        Logger.INFO(f"CRC Bytes: {"DISABLED" if args.crc_bytes == RF24_CRC.DISABLED else "1" if args.crc_bytes == RF24_CRC.BYTES_1 else "2"}")

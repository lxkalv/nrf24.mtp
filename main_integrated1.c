...
#include <zlib.h>
#include <math.h> 
#include <signal.h> // Added for Ctrl+C support in CLI mode
...
bool ui_mode_active = false; // Flag to know if we should blink LEDs
...
// Signal Handler for CLI Ctrl+C
void sigint_handler(int dummy) {
    (void)dummy;
    trigger_reset();
    fprintf(stderr, "\n[CLI] Caught Ctrl+C, stopping safely...\n");
}
...
void toggle_activity_led() {
    if (!ui_mode_active) return; // Don't blink if running from terminal command
...
static int send_with_retries(nrf24_t *radio, const uint8_t *buf, uint8_t len, unsigned int timeout_ms, const char *what, uint64_t *rf_bytes_total, uint64_t *rf_frames_total) {
    ...
    for (;;) {
        ...
        ++attempt;
        if (attempt == 1 || (attempt % 50) == 0) WARN("%s: timeout (no ACK) on attempt %u", what, attempt);
        if (attempt % 500 == 0) {
...
static void page_stream_init(PageStream *ps) { ps->bursts = NULL; ps->count = 0; ps->capacity = 0; }

static void free_burst(Burst *b) {
    if (!b) return;
    for (unsigned i = 0; i < b->frames_in_burst; ++i) free(b->frame_data[i]);
    b->frames_in_burst = 0;
}
...
static void page_stream_free(PageStream *ps) {
    ...
    free(ps->bursts); ps->bursts = NULL; ps->count = 0; ps->capacity = 0;
}
...
static Burst *page_get_burst(PageStream *ps, unsigned burst_id) {
    if (burst_id >= ps->capacity) {
        ...
        free(ps->bursts); ps->bursts = new_bursts; ps->capacity = new_cap;
    }
    if (burst_id >= ps->count) {
        for (size_t i = ps->count; i <= burst_id; ++i) {
            ps->bursts[i].frames_in_burst = 0;
            for (unsigned j = 0; j < MAX_CHUNKS_PER_BURST; ++j) ps->bursts[i].frame_data[j] = NULL;
        }
        ps->count = burst_id + 1;
    }
...
static int decompress_page_to_file(PageStream *ps, FILE *fout, uint64_t *compressed_total, uint64_t *uncompressed_total) {
    ...
    for (size_t bid = 0; bid < ps->count && !end_reached; ++bid) {
        ...
        for (unsigned i = 0; i < b->frames_in_burst && !end_reached; ++i) {
            ...
            const uint8_t *in = frame + 1; size_t in_len = b->frame_len[i] - 1;
            *compressed_total += in_len;
            ...
            while (zs.avail_in > 0) {
                ...
                if (have > 0) { fwrite(outbuf, 1, have, fout); *uncompressed_total += have; }
...
static int run_tx(const char *spi_dev, int ce_bcm, const char *input_path) {
    ...
    for (unsigned page_id = 0; page_id < P2P_NUM_PAGES; ++page_id) {
        ...
        size_t comp_pos = 0; uint8_t burst_id = 0;
        while (comp_pos < comp_len) {
            ...
            while (!burst_done) {
                ...
                double w_start = now_seconds(); int got_chk = 0;
                ...
                else { WARN("Checksum timeout, resending burst"); nrf24_set_mode_tx(&radio); }
            }
            nrf24_set_mode_tx(&radio); burst_id++;
        }
        free(comp_page);
    }
...
static int run_rx(const char *spi_dev, int ce_bcm, const char *output_path) {
    ...
    while (!transfer_finished) {
        ...
        uint8_t buf[32]; uint8_t len = 32;
        int ret = nrf24_recv_blocking(&radio, buf, &len, 20);
...
int main(int argc, char **argv) {
    ...
    if (argc > 1) {
        ...
        // Handle Ctrl+C safely in CLI mode
        signal(SIGINT, sigint_handler);

        const char *mode = argv[1];
        const char *spi  = argv[2];
        int ce = atoi(argv[3]);
        const char *path = argv[4];

        ui_mode_active = false; // Disable LED blinking logic
        log_init("radio_cli.log");

        int ret = 1;
        if (strcmp(mode, "tx") == 0) ret = run_tx(spi, ce, path);
        else if (strcmp(mode, "rx") == 0) ret = run_rx(spi, ce, path);
        else usage(argv[0]);

        return ret;
    }

    // --- MODE 2: UI BUTTON/SWITCH INTERFACE ---
    ui_mode_active = true;
    ...
    while(1) {
        ...
        if (is_tx) run_tx("/dev/spidev0.0", 22, file_path);
        else run_rx("/dev/spidev0.0", 22, file_path);
...
//To compile: gcc -o radio_app main_integrated1.c libs/nrf24.c libs/utils.c -lwiringPi -lz -lm

//Method 1
//# Transmitter
//sudo ./radio_app tx /dev/spidev0.0 22 myfile.txt

//# Receiver
//sudo ./radio_app rx /dev/spidev0.0 22 output.txt

...
# ---- NUEVO: helper para normalizar a UTF-8 ----
def read_file_as_utf8_bytes(file_path: Path, add_utf8_bom: bool = True) -> bytes:
    raw = file_path.read_bytes()

    # Detección simple por BOM
    if raw.startswith(codecs.BOM_UTF8):
        ...
        # Sin BOM: probamos UTF-8 → cp1252 como fallback “seguro”
        try:
            text = raw.decode("utf-8", errors="strict")
        except UnicodeDecodeError:
            text = raw.decode("cp1252", errors="strict")

    out = text.encode("utf-8", errors="strict")
    if add_utf8_bom:
...
def BEGIN_TRANSMITTER_MODE() -> None:
    ...
    try:
        ...
        content = read_file_as_utf8_bytes(file_path, add_utf8_bom=True)
        content_len = len(content)
        INFO(f"Read {content_len} bytes (normalized to UTF-8) from {file_path}")

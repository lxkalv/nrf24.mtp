...
#define MODE_SWITCH_TX_DELAY_MS   100
#define MODE_SWITCH_RX_DELAY_MS   20
                                     ...
                                     uint32_t original_len)
{
    ...
    if (compressed_len != 0 || original_len == 0) {
        if (decompress_buffer(compressed,
            ...
            logger_error("p2p RX: failed to decompress payload (%zu bytes)",
                         compressed_len);
            return -1;
        }
    } else {
        logger_error("p2p RX: non-empty output requested but compressed buffer is empty");
        return -1;
    }
    ...
    if (app_store_file_bytes(cfg ? cfg->file_path_rx : NULL,
                             output,
                             (size_t)original_len) != 0) {
        logger_error("p2p RX: failed to store output bytes");
        free(output);
        return -1;
    }

    free(output);
    logger_succ("p2p RX: stored '%s' (%u bytes)", dest_label, original_len);
    return 0;
}
                            ...
                            uint64_t *rf_frames_total)
{
    ...
    int ret = send_with_retries(radio,
                                ...
                                rf_frames_total);
    if (ret == 0) {
        logger_succ("p2p TX: STREAM_INFO acknowledged (frames=%u, comp=%u, orig=%u)",
                    total_frames,
                    comp_len,
                    orig_len);
    }
                  ...
                  size_t file_len)
{
    ...
    if (compressed_len > UINT32_MAX || file_len > UINT32_MAX) {
        logger_error("File too large for p2p_mode (limit 4 GiB per stage)");
        goto cleanup;
    }
    ...
    logger_info("p2p TX: '%s' -> %zu bytes compressed (%u frames, %u ID bytes)",
                input_label,
                compressed_len,
    ...
    while (!transfer_complete) {
        ...
        logger_info("p2p TX: sending data (round %u)", resend_round + 1);
        if (payload_bytes > 0) {
        ...
        for (uint32_t frame = 0; frame < total_frames; ++frame) {
            ...
            if (total_frames > 0) {
                ...
                if (pct >= next_tx_progress_pct) {
                    logger_info("p2p TX: progress %u%% (%u/%u frames)",
                                pct,
                                frame + 1,
                                total_frames);
        ...
        logger_info("p2p TX: waiting for checksum reply");
        int checksum_ok = 0;
        double wait_start = now_seconds();
        while (!checksum_ok && (now_seconds() - wait_start) * 1000.0 < CHECKSUM_TIMEOUT_MS) {
            ...
            if (ret < 0) {
                ...
                logger_error("p2p TX: nrf24_recv_blocking failed: %s", strerror(errno));
                nrf24_deinit(&radio);
                goto cleanup;
            }

            if (len < 2) {
                logger_warn("p2p TX: short frame len=%u while waiting for checksum", len);
                continue;
            }

            if (buf[0] != CONTROL_PREFIX) {
                logger_warn("p2p TX: unexpected data frame during checksum wait");
                continue;
            }

            if (buf[1] == MSG_CHECKSUM) {
                if (len < 2 + CHECKSUM_SIZE) {
                    logger_warn("p2p TX: malformed CHECKSUM frame");
                    continue;
                }
                ...
                logger_warn("p2p TX: checksum mismatch (expected 0x%016llX, got 0x%016llX)",
            ...
            if (buf[1] == MSG_STREAM_INFO) {
                logger_warn("p2p TX: RX resent STREAM_INFO ack; ignoring");
                continue;
            }

            if (buf[1] == MSG_STREAM_FINISH) {
                logger_info("p2p TX: RX already finished transfer");
                checksum_ok = 1;
                break;
            }
        }
        ...
        if (!checksum_ok) {
            logger_warn("p2p TX: checksum not confirmed, resending data");
            if (ensure_mode_tx(&radio) != 0) {
                nrf24_deinit(&radio);
                goto cleanup;
            }
    ...
    logger_succ("p2p TX: transfer complete (%zu bytes -> %zu bytes)",
                file_len,
                compressed_len);
    nrf24_deinit(&radio);
    exit_code = 0;

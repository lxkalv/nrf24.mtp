...
static void log_page_burst_layout(const char *context, unsigned page_id, const PageBurstLayout *layout) {
    if (!layout) {
        return;
    }
...
static int run_tx(const char *spi_dev, int ce_bcm, const char *input_path, const app_config_t *cfg) {
